...
There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Use an array method to fix this issue and console.log() to check your work. */
  ...
  function editName (array, index, string) {
    array[index].name = string
    return array[index].name
  }

  editName(artists, 8, 'Vincent Van Gogh')
...
function get20s(array){
  ...
  for (let i = 0; i < artists.length; i++){
    const startYear = parseInt(array[i].years)
    if (startYear >= 1900){
      years20.push(array[i].name)
    }
  }
  return years20
}
...
function lotsOfArt(array){
  const plusHundredClub = []
  for (let i = 0; i < array.length; i++){
    if (array[i].paintings > 100){
      plusHundredClub.push(array[i].name)
    }
  } 
  return plusHundredClub
}
